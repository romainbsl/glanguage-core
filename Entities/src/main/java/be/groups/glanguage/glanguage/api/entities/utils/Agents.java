...
public class Agents {
    ...
    private Method findMethodIncludingInherited(Class<?> currentClass, Class<?>[] methodParameterTypes) throws NoSuchMethodException {
        ...
        while (currentClass != null) {
            lastException = null;
            Class[] types = methodParameterTypes == null ? null :
                    (Class[]) Arrays.stream(methodParameterTypes).map(c -> c = Object.class).toArray(size -> new Class[size]);
            ...
            try {
                ...
                try {
                    // Recherche parmis l'ensemble des méthodes "public" de la classe et super classe.
                    result = currentClass.getMethod(methodName, types);
                    break;
                    ...
                    try {
                        ...
                        try {
                            // Recherche parmis l'ensemble des méthodes "private, protected et public" de la classe.
                            result = currentClass.getDeclaredMethod(methodName, types);
                            result.setAccessible(true);
                            break;
