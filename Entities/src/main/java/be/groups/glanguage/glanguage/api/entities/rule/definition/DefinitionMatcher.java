	...
	public static boolean matches(Collection<RuleDefinitionParameter> first, Collection<RuleDefinitionParameter> second) {
		if (first.isEmpty()) {
			...
			if (!socialSecretariatParameters.isEmpty()) {
				match = socialSecretariatParameters.stream().anyMatch(f -> second.stream().anyMatch(s -> s.matches(f)));
			}
			if (match) {
				...
				if (!employerParameters.isEmpty()) {
					match = employerParameters.stream().anyMatch(f -> second.stream().anyMatch(s -> s.matches(f)));
				}
				if (match) {
					...
					if (!jointCommitteeParameters.isEmpty()) {
						match = jointCommitteeParameters.stream()
								.anyMatch(f -> second.stream().anyMatch(s -> s.matches(f)));
					}
					if (match) {
